...
for i in range(0,26):  #Interate alphabets and assign key value pair. Value is i+13 th value.
    if i+13 <= 25:     # checking if last character to replace with (i.e, 'z') is reached or not.
        key[alphabet[i]] = alphabet[i+13]  
        key[alphabet[i].upper()] = key[alphabet[i]].upper()  #Also assign upper case letters.
    else:
        key[alphabet[i]] = alphabet[i-26+13]  #assignment after last character is reached, i.e, from 'a' onwards
        key[alphabet[i].upper()]=key[alphabet[i]].upper()  #upper case
